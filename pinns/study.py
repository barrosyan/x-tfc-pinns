...
# Define ELM regressor
elm = ELMRegressor(ufunc='tanh', alpha=0.5)

real = lambda x: np.array([c/alpha-c/alpha*np.tanh(c*(xi[0]-c*xi[1])/(2.*nu)) for xi in x])
# Reshape x for ELM fitting
x_fit = np.column_stack((x[0].flatten(), x[1].flatten()))
elm.fit(x_fit, real(x_fit))

# Initial guess for xi
xi_initial_guess = elm.predict(x_fit)
...
d2x = egrad(ux, 1)
ut = egrad(u, 2)
r = lambda xi, *x: ut(xi, *x) + alpha * u(xi, *x) * ux(xi, *x) - nu * d2x(xi, *x)

# Solve the problem
xi, it, time = NLLS(xi_initial_guess, r, method='lstsq', timer=True, timerType="perf_counter")

# Calculate error at the test points:
dark = np.meshgrid(np.linspace(xInit, xFinal, nTest), np.linspace(0., 1., nTest))
xTest = tuple([j.flatten() for j in dark])
err = np.abs(u(xi, *xTest) - real(np.column_stack((xTest[0], xTest[1]))))
